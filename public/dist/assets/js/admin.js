...
$('#findBlackList').on('click', async () => {
  ...
  try {
    const response = await axios.get(
      `${adminPort}/user/me/searchEmail/?email=${email}`,
      {
        headers: {
    ...
    $('#createBlackList').on('click', async () => {
      ...
      try {
        await axios.post(`${adminPort}:3000/blacklist`, data, {
...
async function recordPage(page, pageSize) {
  await axios
    .get(`${adminPort}:3000/report?page=${page}&pageSize=${pageSize}`, {
      headers: {
        Authorization: adminToken,
      },
    })
...
async function getReports(page, pageSize) {
  try {
    const { data } = await axios.get(
      `${adminPort}:3000/report?page=${page}&pageSize=${pageSize}`,
      {
        headers: {
          Authorization: adminToken,
        },
  ...
  try {
    ...
    $('#blackuser')
      ...
      .on('click', function () {
        ...
        axios.post(`${adminPort}:3000/blacklist/${userId}`, data, {
          headers: { Authorization: adminToken },
        });
...
async function checkAdmin() {
  try {
    const { data } = await axios.get(`${adminPort}:3000/blacklist/permision`, {
      headers: {
        Authorization: adminToken,
      },
    });
